...
export default async function handler(
  ...
  try {
    ...
    if (debug === "true") {
      ...
      console.log(
        "Contains ytInitialPlayerResponse:",
    ...
    if (!chosenTrack && tracks.length > 0) {
      ...
      console.log(
        `Subtitle for language "${lang}" not found, using first available (${chosenTrack.languageCode})`
      );
    }
    ...
    if (debug === "true") {
      ...
      console.log(
        ...
        Object.fromEntries(subResp.headers.entries())
      );
    }
    ...
    if (debug === "true") {
      ...
      console.log(
        "Raw subtitle content (first 500 chars):",
